...
from lib.models.hiptrack.DCPT import DCPT
from lib.models.layers.head import build_box_head
from lib.models.hiptrack.vit_ce_prompt import vit_base_patch16_224_ce_prompt
   ...
   #+DCP
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224_prompt':
        backbone = vit_base_patch16_224_ce_prompt(
            ...
            search_size=to_2tuple(cfg.DATA.SEARCH.SIZE),#DCPT size=256
            template_size=to_2tuple(cfg.DATA.TEMPLATE.SIZE),#DCPT size=128
    ...
    if ('HIPTrack' in cfg.MODEL.PRETRAIN_FILE or 'DropTrack' in cfg.MODEL.PRETRAIN_FILE) and training:
        ...
        pretrained_path1 = os.path.join(current_dir, '../../../pretrained_models', cfg.MODEL.PRETRAIN_FILE1)
        checkpoint1 = torch.load(pretrained_path1, map_location="cpu")
        # missing_keys1, unexpected_keys1 = model.load_state_dict(checkpoint1["net"], strict=False)
        weights_dict1 = model1.state_dict()

        for k in weights_dict1.keys():
            if k in weights_dict.keys() and not k.startswith('fc'):
                weights_dict[k] = weights_dict1[k]
