...
class BaseTrainer:
    """Base trainer class. Contains functions for training and saving/loading checkpoints1.
    ...
    def update_settings(self, settings=None):
        ...
        if self.settings.env.workspace_dir is not None:
            ...
            if self.settings.save_dir is None:
                self._checkpoint_dir = os.path.join(self.settings.env.workspace_dir, 'checkpoints1')
            else:
                self._checkpoint_dir = os.path.join(self.settings.save_dir, 'checkpoints1')
            print("checkpoints1 will be saved to %s" % self._checkpoint_dir)

            if self.settings.local_rank in [-1, 0]:
                if not os.path.exists(self._checkpoint_dir):
                    print("Training with multiple GPUs. checkpoints1 directory doesn't exist. "
                          "Create checkpoints1 directory")
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    # only save the last 10 checkpoints1
                    save_every_epoch = getattr(self.settings, "save_every_epoch", False)
                    save_epochs = [0, 1, 2, 3, 20,50,79, 159, 239]
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        if ignore_fields is None:
            ...
            # Never load the scheduler. It exists in older checkpoints1.
