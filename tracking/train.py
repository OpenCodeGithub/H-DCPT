...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, help='root directory to save checkpoints1, logs, and tensorboard')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python /home/lsw/PycharmProjects/HIPTrack_Re/H-DCPT/lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d /home/lsw/PycharmProjects/HIPTrack_Re/H-DCPT/lib/train/run_training.py " \
